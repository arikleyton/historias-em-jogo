// =====================================================================
// Capitulo 2 — A Travessia do Atlantico e o Desvio (Volta do Mar)
// Serious Game — IFPA — Descobrimento do Brasil
...
//   constava nos mapas — o futuro Brasil.
//
...
//   HowToPlayScene -> instrucoes de como jogar
//
// Mecanica central (o que o jogo esta realmente ensinando):
//   O navio avanca sozinho para cima (rumo ao território desconhecido) e o
...
// nao disparam o evento 'error' acima — precisam ser tratados a parte,
...
const GAME = {
  ...
  // oceano — e nao apenas um cenario decorativo passando ao fundo.
  worldWidth: 1280,
  worldHeight: 22000,
  ...
  // curta (8200), o navio completava a travessia cedo demais — a
...
// IM Fell English para textos corridos — reforcam a leitura "manuscrito
...
// corrente concedem o mesmo boost compartilhado ao serem capturados. Dentro
// da neblina, somente parte do bonus do vento favoravel continua atuando.
const BOOST = {
...
const FOG_EFFECT = {
  ...
  windBoostRetention: 0.40,
  obstacleRevealStart: 285,
  obstacleRevealFull: 120
};
...
// ---------------------------------------------------------------------
const AUDIO_KEYS = {
  menuMusic: 'audio-musica-menu',
  gameplayMusic: 'musicaGameplay',
  gameplayMusicAlt: 'musicaGameplayAlt',
...
class GameAudio {
  constructor() {
    ...
    this.mode = 'silent';
    this.rainMode = 'none';
    this.gameplayMusicKeys = [AUDIO_KEYS.gameplayMusic, AUDIO_KEYS.gameplayMusicAlt];
    this.currentGameplayMusic = null;
  }

  bind(scene) {
    if (!scene || !scene.sound || !scene.cache || !scene.cache.audio) return;
    const definitions = [
      [AUDIO_KEYS.menuMusic, { loop: true, volume: 0.20 }],
      [AUDIO_KEYS.gameplayMusic, { loop: true, volume: 0.14 }],
      [AUDIO_KEYS.gameplayMusicAlt, { loop: true, volume: 0.14 }],
  ...
  applyMode(scene) {
    ...
    this.bind(scene);
    const activeGameplayMusic = this.currentGameplayMusic || this.gameplayMusicKeys[0];
    this.gameplayMusicKeys.forEach((key) => {
      if (this.mode === 'gameplay' && key === activeGameplayMusic) this.playLoop(key, 0.14);
      else this.stop(key);
    });
    if (this.mode === 'menu') {
      this.stop(AUDIO_KEYS.roughSea);
      this.playLoop(AUDIO_KEYS.calmSea, 0.10);
      this.playLoop(AUDIO_KEYS.distantGulls, 0.035);
      this.playLoop(AUDIO_KEYS.menuMusic, 0.20);
    } else if (this.mode === 'calm') {
      this.stop(AUDIO_KEYS.menuMusic);
      this.stop(AUDIO_KEYS.roughSea);
      this.playLoop(AUDIO_KEYS.calmSea, 0.12);
      this.playLoop(AUDIO_KEYS.distantGulls, 0.055);
  ...
  setMode(scene, mode) {
    ...
    const unlock = () => {
      ...
      if (this.mode === mode) scene.time.delayedCall(0, () => this.applyMode(scene));
    };
    if (scene && scene.sound) scene.sound.once('unlocked', unlock);
  ...
  enterGameplay(scene) {
    const choices = this.gameplayMusicKeys.filter((key) => key !== this.currentGameplayMusic);
    this.currentGameplayMusic = choices[Math.floor(Math.random() * choices.length)] || this.gameplayMusicKeys[0];
    this.setMode(scene, 'gameplay');
  }
...
// Painel de pergaminho usado nos textos de menu/derrota/vitoria/instrucoes e
...
// visual "Historical Comic Art" em todas as cenas — o jogo deve parecer a
...
// =====================================================================
// BootScene — carrega os assets enviados e gera as texturas restantes
...
class BootScene extends Phaser.Scene {
  ...
  preload() {
    // O jogo publicado usa somente os spritesheets atuais. Caso um deles
    // falhe, buildTextures cria uma versao vetorial de seguranca por codigo.
    ...
    this.load.audio(AUDIO_KEYS.menuMusic, assetUrl('audio/musica-menu-45s.mp3'));
    this.load.audio(AUDIO_KEYS.gameplayMusic, assetUrl('audio/musica-gameplay.mp3'));
    this.load.audio(AUDIO_KEYS.gameplayMusicAlt, assetUrl('audio/musica-gameplay-2.mp3'));
  ...
  buildTextures() {
    ...
    // Cruz da Ordem de Cristo — simbolo classico das caravelas
    ...
    // Gerado em baixa resolucao e esticado via setDisplaySize — como e
    ...
    // Fragmento bem pequeno: quase rente a agua, com formato triangular.
    g.clear();
    g.fillStyle(0xdfe9ee, 0.18);
    g.fillEllipse(36, 50, 68, 25);
    g.beginPath();
    g.moveTo(7, 53);
    g.lineTo(24, 31);
    g.lineTo(34, 8);
    ...
    g.lineTo(66, 54);
    g.closePath();
    g.fillStyle(0x555c59, 1);
    g.fillPath();
    g.lineStyle(3, 0x171919, 0.9);
    g.strokePath();
    g.fillStyle(0x919995, 0.78);
    g.fillTriangle(24, 31, 34, 9, 39, 36);
    g.generateTexture('rock-tiny', 72, 64);

    // Agulha rochosa: alta, estreita e inclinada, bem diferente das massas
    // arredondadas. A hitbox permanece menor que o desenho.
    g.clear();
    g.fillStyle(0xdfe9ee, 0.17);
    g.fillEllipse(52, 124, 100, 30);
    g.beginPath();
    g.moveTo(14, 126);
    g.lineTo(31, 82);
    g.lineTo(42, 55);
    ...
    g.lineTo(91, 127);
    g.closePath();
    g.fillStyle(0x3f4644, 1);
    g.fillPath();
    g.lineStyle(3, 0x151717, 0.9);
    g.strokePath();
    g.fillStyle(0x858e89, 0.82);
    g.fillTriangle(42, 56, 61, 8, 58, 75);
    g.fillStyle(0x292e2d, 0.76);
    g.fillTriangle(61, 70, 70, 63, 82, 111);
    g.generateTexture('rock-spire', 104, 142);

    // Recife baixo e largo: varias pontas separadas formam uma barreira
    // horizontal, deixando a silhueta bem distinta das pedras altas.
    g.clear();
    g.fillStyle(0xdfe9ee, 0.17);
    g.fillEllipse(110, 75, 214, 35);
    g.beginPath();
    g.moveTo(4, 78);
    g.lineTo(27, 48);
    g.lineTo(52, 65);
    ...
    g.lineTo(216, 79);
    g.closePath();
    g.fillStyle(0x464c4a, 1);
    g.fillPath();
    g.lineStyle(3, 0x161818, 0.9);
    g.strokePath();
    g.fillStyle(0x858d89, 0.75);
    g.fillTriangle(54, 62, 78, 25, 88, 65);
    g.fillTriangle(154, 62, 182, 32, 190, 69);
    g.generateTexture('rock-reef', 220, 94);

    // Picos gemeos assimetricos: duas massas grandes com um canal no meio.
    g.clear();
    g.fillStyle(0xdfe9ee, 0.17);
    g.fillEllipse(88, 122, 170, 38);
    g.beginPath();
    g.moveTo(7, 126);
    g.lineTo(29, 76);
    g.lineTo(54, 17);
    ...
    g.lineTo(169, 127);
    g.closePath();
    g.fillStyle(0x414745, 1);
    g.fillPath();
    g.lineStyle(3, 0x151717, 0.9);
    g.strokePath();
    g.fillStyle(0x89918d, 0.78);
    g.fillTriangle(29, 76, 54, 18, 59, 82);
    g.fillTriangle(113, 66, 136, 36, 143, 95);
    g.fillStyle(0x292e2d, 0.72);
    g.fillTriangle(77, 91, 92, 112, 111, 68);
    g.generateTexture('rock-twin', 176, 142);

    // Se o spritesheet atual nao for carregado, geramos uma versao de
...
// =====================================================================
// AmbientBackdrop — cenario decorativo animado com aves para as
// cenas que NAO rolam camera (Menu, Intro, Falha, Vitoria, Como Jogar).
// =====================================================================
  ...
  constructor(scene, alpha = 1) {
    ...
    for (let i = 0; i < 7; i++) {
      const bird = scene.add.image(
        ...
        'bird'
      ).setDepth(3).setAlpha(0.38).setScale(Phaser.Math.FloatBetween(0.55, 1.05));
...
class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    // curta que situa o jogador antes da missao.
    ...
    const missionOffsetX = 245;
    shadowedPaper(40 + missionOffsetX, 154, 710, 390);
    const missionRibbon = this.add.graphics().setDepth(25).setScrollFactor(0).setPosition(missionOffsetX, 0);
    missionRibbon.fillStyle(0x7d3f23, 1);
    missionRibbon.fillRect(58, 172, 244, 34);
    missionRibbon.fillTriangle(302, 172, 324, 189, 302, 206);
    makeText(this, 74 + missionOffsetX, 189, 'A MISSÃO  •  7.000 KM', {
      font: FONT_TITLE, size: '15px', style: 'bold', color: '#fff4cf', depth: 26
    }).setOrigin(0, 0.5);
    makeText(this, 64 + missionOffsetX, 222,
    ...
    const map = this.add.graphics().setDepth(24).setScrollFactor(0).setPosition(missionOffsetX, 0);
    ...
    makeText(this, 576 + missionOffsetX, 326, 'CABO VERDE', { size: '11px', style: 'bold', color: '#3b2a18', depth: 27 });
    makeText(this, 632 + missionOffsetX, 482, 'ÁFRICA', { size: '12px', style: 'bold', color: '#3b2a18', depth: 27 });
    makeText(this, 84 + missionOffsetX, 472, 'COSTA\nDESCONHECIDA', { size: '11px', style: 'bold', color: '#27311e', align: 'center', depth: 27 });
    makeText(this, 344 + missionOffsetX, 452, 'VOLTA DO MAR', {
      font: FONT_TITLE, size: '14px', style: 'bold', color: '#7d3f23', depth: 27
    }).setAngle(-7);
    const mapShip = this.add.image(395 + missionOffsetX, 420, 'caravela-side')
    ...
    const compass = this.add.image(694 + missionOffsetX, 474, 'compass-rose')
    ...
    makeMenuButton(954, 582, 178, 50, 'COMO JOGAR', () => goToScene('HowToPlayScene'));
...
// =====================================================================
// OceanWorld — cenario da travessia: gradiente do oceano (mundo todo),
// agua ambiente animada, aves e a revelacao da costa no final.
// =====================================================================
class OceanWorld {
  constructor(scene) {
    this.scene = scene;
    ...
    for (let i = 0; i < 16; i++) {
      const speck = scene.add.image(
        ...
        .setAlpha(Phaser.Math.FloatBetween(0.10, 0.24))
    ...
    // Aves decorativas fixas na camera, sem nuvens claras sobre a gameplay.
    this.birds = [];
    for (let i = 0; i < 8; i++) {
      const bird = scene.add.image(
        Phaser.Math.Between(0, GAME.width),
        Phaser.Math.Between(65, 280),
        'bird'
      ).setDepth(3).setScrollFactor(0).setAlpha(0.40).setScale(Phaser.Math.FloatBetween(0.50, 1.08));
      this.birds.push(bird);
    }
    ...
    // aproxima da costa (ultimos ~22% da travessia) — reforco
  ...
  update(delta, forwardSpeed, progress = 0) {
    ...
    this.foamTile.setAlpha(0.20 + progress * 0.10);
    ...
    this.birds.forEach((bird, i) => {
      bird.x -= (0.58 + i * 0.065) * (delta / 16.67);
      bird.y += Math.sin((this.scene.time.now + i * 300) / 450) * 0.16;
      if (bird.x < -80) { bird.x = GAME.width + 80; bird.y = Phaser.Math.Between(65, 280); }
    });
  }
}

// =====================================================================
// LogbookHUD — "Diario de Bordo", com barra de progresso VERTICAL
...
// =====================================================================
// MessageLayer — legendas narrativas temporizadas (fixas na camera)
// =====================================================================
class MessageLayer {
  constructor(scene) {
    this.scene = scene;
...
// =====================================================================
// ShipController — movimento vertical automatico + direcao lateral do
...
class ShipController {
  ...
  pushFromWave(sourceX) {
    ...
    this.velocityX += dir * 2.6;
    this.impulseX += dir * 7.2;
    this.controlPenalty = 1700;
  }
}

// =====================================================================
// EventTrack — eventos da travessia pre-posicionados no mundo (faixas de
...
class EventTrack {
  ...
  // Mapa ALEATORIO (gerado a cada partida) — bem mais denso e perigoso
  ...
  buildTrack() {
    ...
    // Dificuldade moderadamente maior: mais perigos no total e uma presenca
    // bem maior de ondas, sem reduzir os vaos seguros das tempestades.
    const weightedTypes = [
      'wind', 'wind', 'current', 'current',
      'wave', 'wave', 'wave', 'fog',
      'rock', 'rock', 'rock', 'rock', 'rock', 'rock', 'rock', 'rock',
      'whirlpool', 'whirlpool', 'whirlpool', 'whirlpool', 'whirlpool', 'whirlpool'
    ];

    // Regras de espacamento — calibradas pra sempre sobrar tempo real de
    ...
    //    anterior (incluindo vento/corrente) e uma tempestade — evita o
    ...
    //    proximo — impede dois perigos seguidos em lados opostos da
    ...
    while (traveled < totalDistance - margin && guard < 150) {
      guard++;
      traveled += Phaser.Math.Between(115, 185);
      ...
      const w = { wind: 58, current: 58, wave: 150, fog: 230, rock: 220, whirlpool: 188 }[type];
  ...
  spawnGate(worldY, type, x) {
    const rockVariants = [
      { key: 'rock-tiny', w: 54, h: 48, hitW: 36, hitH: 32, score: -10 },
      { key: 'rock-tiny', w: 54, h: 48, hitW: 36, hitH: 32, score: -10 },
      { key: 'rock-small', w: 72, h: 68, hitW: 48, hitH: 46, score: -10 },
      { key: 'rock-small', w: 72, h: 68, hitW: 48, hitH: 46, score: -10 },
      { key: 'rock', w: 100, h: 94, hitW: 76, hitH: 72, score: -10 },
      { key: 'rock-spire', w: 92, h: 126, hitW: 58, hitH: 96, score: -10 },
      { key: 'rock-large', w: 150, h: 104, hitW: 106, hitH: 82, score: -10 },
      { key: 'rock-twin', w: 158, h: 122, hitW: 108, hitH: 88, score: -10 },
      { key: 'rock-reef', w: 200, h: 82, hitW: 142, hitH: 50, score: -10 }
    ];
  ...
  // com um vao visivel de verdade entre elas — antes o vao so existia na
  ...
  spawnStorm(worldY, height, gap, gapX, guaranteed = false) {
    ...
    // Varios raios espalhados pelas duas metades da nuvem (antes era só
    // um, perto do vão) — pedido: mais raios em cima da nuvem.
    const lightnings = [
  ...
  // Colisao/coleta das faixas de vento, corrente, pedras e redemoinhos —
  ...
  updateGates(shipBounds, shipY, callbacks) {
    this.gates.forEach((gate) => {
      ...
      if (isHazard) {
        const fogStrength = this.scene.fogStrength || 0;
        if (fogStrength > 0 && dist > 0) {
          const proximity = 1 - Phaser.Math.Clamp(
            (dist - FOG_EFFECT.obstacleRevealFull) /
            (FOG_EFFECT.obstacleRevealStart - FOG_EFFECT.obstacleRevealFull),
            0,
            1
          );
          const foggedAlpha = Phaser.Math.Linear(0.01, 1, proximity);
          gate.sprite.setAlpha(Phaser.Math.Linear(1, foggedAlpha, fogStrength));
          gate.fogDimmed = true;
        } else if (gate.fogDimmed) {
          gate.fogDimmed = false;
          gate.sprite.setAlpha(gate.type === 'whirlpool' ? 0.94 : 1);
        }
      }
  ...
  updateStorms(ship) {
    ...
    this.storms.forEach((storm) => {
      ...
      if (fogStrength > 0) {
        const proximity = 1 - Phaser.Math.Clamp(
          (distToBand - FOG_EFFECT.obstacleRevealFull) /
          (FOG_EFFECT.obstacleRevealStart - FOG_EFFECT.obstacleRevealFull),
          0,
          1
        );
        const visibility = Phaser.Math.Linear(1, Phaser.Math.Linear(0.01, 1, proximity), fogStrength);
        storm.sprites.forEach((sprite, index) => sprite.setAlpha(visibility * (index === 2 ? 0.18 : 1)));
        storm.fogDimmed = true;
      } else if (storm.fogDimmed) {
        storm.fogDimmed = false;
        storm.sprites.forEach((sprite, index) => sprite.setAlpha(index === 2 ? 0.18 : 1));
      }
      ...
      // perfeito — sobra bastante espaco transparente perto das bordas da
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // OBS: nao usar cameras.main.setZoom() aqui — a HUD/UI inteira usa
    ...
    // uma segunda camera dedicada so pra UI — nao vale a complexidade
    ...
    this.fogOverlay.lineStyle(3, 0xffffff, 0.30);
    for (let y = 55; y < GAME.height; y += 72) {
  ...
  // "claro" e "nublado/tempestuoso" (apenas visual — a dificuldade real
  ...
  updateWeather() {
    // this.traveled nunca diminui, entao a faixa atual so pode avancar —
  ...
  // boost compartilhado por alguns segundos ao serem capturados; na neblina,
  // o vento preserva uma parcela menor desse bonus. A
  // velocidade "de base" cresce sozinha e aos poucos ao longo de toda a
  // travessia. Pedra, redemoinho e tempestade sao os riscos fatais — o
  ...
  //    suavemente — nunca cai de uma vez);
  ...
  updateSpeed(delta) {
    ...
    if (this.boostTimer > 0) {
      ...
      if (this.boostMultiplier < 1.01) {
        this.boostMultiplier = 1;
        this.boostSource = 'none';
      }
    }

    const baseTargetSpeed = BASE_SPEED.start * this.baseSpeedMultiplier;
    const normalTargetSpeed = baseTargetSpeed * this.boostMultiplier;
    const fogBaseMultiplier = Phaser.Math.Linear(1, FOG_EFFECT.speedMultiplier, this.fogStrength);
    const retainedWindBonus = this.boostSource === 'wind'
      ? (this.boostMultiplier - 1) * FOG_EFFECT.windBoostRetention
      : 0;
    const fogTargetSpeed = baseTargetSpeed * fogBaseMultiplier * (1 + retainedWindBonus);
    const targetSpeed = this.fogStrength > 0 ? Math.min(normalTargetSpeed, fogTargetSpeed) : normalTargetSpeed;
  ...
  applyFog() {
    if (this.fogTimer <= 0) this.fogElapsed = 0;
    this.fogTimer = FOG_EFFECT.durationMs;
    SFX.fog();
    GAME_AUDIO.effect(this, AUDIO_KEYS.coldWind, 0.16, FOG_EFFECT.durationMs);
  ...
  update(time, delta) {
    ...
    // calculada de uma vez em updateSpeed — ver o comentario ali pra
    ...
    this.fogElapsed = this.fogTimer > 0 ? this.fogElapsed + delta : 0;
    this.waveFlash = Math.max(0, this.waveFlash - delta);
    const fogFadeIn = Phaser.Math.Clamp(this.fogElapsed / FOG_EFFECT.fadeInMs, 0, 1);
    const fogFadeOut = Phaser.Math.Clamp(this.fogTimer / FOG_EFFECT.fadeOutMs, 0, 1);
    this.fogStrength = this.fogTimer > 0 ? Math.min(fogFadeIn, fogFadeOut) : 0;
    const fogPulse = 0.97 + Math.sin(time / 360) * 0.03;
    const fogAlpha = FOG_EFFECT.maxAlpha * this.fogStrength * fogPulse;
    ...
    // minima da travessia seja cumprida — garante >= 1 minuto de jogo
    ...
    // o navio subir sozinho — mas com o navio mais rapido agora, isso
    ...
    // conforme ele se aproxima do fim — ele nunca desaparece de vista.
  ...
  stateForHUD() {
    ...
    return {
      ...
      speedMultiplier: this.forwardSpeed / BASE_SPEED.start,
...
// =====================================================================
// FailureScene — tela de naufragio
// =====================================================================
class FailureScene extends Phaser.Scene {
  constructor() { super('FailureScene'); }

  init(data) {
    this.reason = (data && data.reason) || 'O navio não resistiu à travessia.';
  ...
  create() {
    ...
    makeText(this, 300, 146, 'NAUFRÁGIO!', { font: FONT_TITLE, size: '32px', style: 'bold', color: '#5a1f16', depth: 22 });
    makeText(this, 300, 206, this.reason, { size: '20px', color: '#241a10', wrap: 660, lineSpacing: 8, depth: 22 });
    makeText(this, 300, 288,
      'A Volta do Mar exigia cálculo: afastar-se demais da costa, sem cuidado ' +
      'com ventos, correntes e tempestades, colocava toda a expedição em risco.',
      { size: '18px', color: '#4a3d2c', wrap: 660, lineSpacing: 8, depth: 22 }
    );
    makeText(this, 300, 348, `Pontuação até o naufrágio: ${this.finalScore}`, {
    ...
    // Mantem a integracao feita pelo professor: o aluno pode seguir para
    // a chegada mesmo depois de um naufragio, sem perder a mensagem
    // historica de "Terra a vista".
    makeButton(this, 300, 468, 340, 50, 'PULAR PARA A HISTÓRIA', () => {
      this.scene.start('ArrivalScene', {
...
class VictoryScene extends Phaser.Scene {
  ...
  create() {
    ...
    makeText(this, 640, 344, 'TERRA À VISTA!', {
    ...
    makeText(this, 220, 390,
      ...
      'levou a esquadra para oeste do Atlântico. A manobra afastou os navios da costa ' +
      'africana e acabou aproximando Cabral de uma terra que não aparecia nos mapas europeus.',
      { size: '18px', color: '#241a10', wrap: 840, lineSpacing: 7, depth: 22 }
    );

    makeText(this, 220, 500,
      'Resumo: você aproveitou ventos e correntes favoráveis, desviou de riscos do Atlântico e completou o desvio para oeste.',
      { size: '17px', color: '#4a3d2c', wrap: 840, lineSpacing: 5, depth: 22 }
    );

    makeText(this, 220, 548, `Distância didática: ${this.distanceKm.toLocaleString('pt-BR')} km  -  Tempo: ${this.finalTime}s  -  Pontuação: ${this.finalScore}`, {
      font: FONT_TITLE, size: '17px', style: 'bold', color: '#5f4420', depth: 22
    });

    makeButton(this, 320, 642, 280, 46, 'PRÓXIMO CAPÍTULO', () => {
      window.location.href = '../03/cap03.html';
    });
...
// =====================================================================
// HowToPlayScene — instrucoes do minigame
// =====================================================================
class HowToPlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    const shadowedPaper = (x, y, w, h, depth = 20) => {
      const shadow = this.add.graphics().setDepth(depth).setScrollFactor(0);
      shadow.fillStyle(0x000000, 0.38);
      shadow.fillRoundedRect(x + 8, y + 10, w, h, 16);
      drawPaper(this, x, y, w, h, depth + 1, 0.97);
    ...
    makeText(this, 640, 50, 'COMO JOGAR', {
      font: FONT_TITLE, size: '30px', style: 'bold', color: '#fff4cf', align: 'center', depth: 22
    }).setOrigin(0.5);

    // O mesmo Diario de Bordo que antes ocupava a tela inicial.
    shadowedPaper(370, 80, 540, 430);
    makeText(this, 430, 120, 'DIÁRIO DE BORDO', {
      font: FONT_TITLE, size: '21px', style: 'bold', color: '#241a10', depth: 30
    });
    makeText(this, 834, 124, 'COMO NAVEGAR', {
      size: '11px', style: 'bold', color: '#7d3f23', align: 'right', depth: 30
    }).setOrigin(1, 0);

    makeKeycap(460, 151, 'W');
    makeKeycap(427, 180, 'A'); makeKeycap(460, 180, 'S'); makeKeycap(493, 180, 'D');
    makeText(this, 475, 215, 'WASD', {
      size: '11px', style: 'bold', color: '#5b3b1e', align: 'center', depth: 32
    }).setOrigin(0.5);

    makeKeycap(585, 151, '↑');
    makeKeycap(552, 180, '←'); makeKeycap(585, 180, '↓'); makeKeycap(618, 180, '→');
    makeText(this, 600, 215, 'SETAS', {
      size: '11px', style: 'bold', color: '#5b3b1e', align: 'center', depth: 32
    }).setOrigin(0.5);

    const touchBadge = this.add.graphics().setDepth(31).setScrollFactor(0);
    touchBadge.fillStyle(0x183f52, 0.12);
    touchBadge.fillRoundedRect(694, 150, 144, 64, 9);
    touchBadge.lineStyle(1.5, 0x7d5a28, 0.55);
    touchBadge.strokeRoundedRect(694, 150, 144, 64, 9);
    makeText(this, 708, 160, '☝', { size: '28px', color: '#2f2418', depth: 32 });
    makeText(this, 748, 160, 'TOQUE', { size: '12px', style: 'bold', color: '#2f2418', depth: 32 });
    makeText(this, 748, 181, 'e arraste', { size: '12px', color: '#5b3b1e', depth: 32 });
    makeText(this, 430, 232, 'O navio avança sozinho. Guie para os lados e escolha uma rota segura.', {
      size: '12px', color: '#4d3b27', depth: 32
    });

    const divider = this.add.graphics().setDepth(30).setScrollFactor(0);
    divider.lineStyle(2, 0x7d5a28, 0.38);
    divider.lineBetween(430, 258, 848, 258);
    makeText(this, 430, 269, 'LEGENDA DOS ELEMENTOS', {
      font: FONT_TITLE, size: '15px', style: 'bold', color: '#241a10', depth: 32
    });
    const legend = [
      { key: 'icon-wind', label: 'VENTO', note: '+ velocidade', x: 438, y: 306, good: true },
      { key: 'icon-current', label: 'CORRENTE', note: '+ velocidade', x: 650, y: 306, good: true },
      { key: VISUAL_ASSETS.wave, animation: ASSET_ANIMATIONS.wave, label: 'ONDA', note: 'empurra', x: 438, y: 351 },
      { key: 'fog-bank', label: 'NEBLINA', note: 'encobre', x: 650, y: 351 },
      { key: 'rock', label: 'PEDRAS', note: 'naufrágio', x: 438, y: 396, danger: true },
      { key: VISUAL_ASSETS.whirlpool, animation: ASSET_ANIMATIONS.whirlpool, label: 'REDEMOINHO', note: 'naufrágio', x: 650, y: 396, danger: true },
      { key: 'icon-storm', label: 'TEMPESTADE', note: 'naufrágio', x: 438, y: 441, danger: true }
    ];
    legend.forEach((item) => {
      const icon = this.add.sprite(item.x, item.y, item.key).setDepth(25).setScrollFactor(0);
      ...
      makeText(this, item.x + 25, item.y - 12, item.label, {
        size: '12px', style: 'bold', color: '#2f2418', depth: 33
      });
      makeText(this, item.x + 25, item.y + 5, item.note, {
        size: '11px', style: item.danger ? 'bold' : 'normal',
    ...
    lifeSeal.fillRoundedRect(638, 428, 204, 38, 8);
    lifeSeal.lineStyle(1.5, 0x7d3f23, 0.65);
    lifeSeal.strokeRoundedRect(638, 428, 204, 38, 8);
    makeText(this, 740, 447, '⚠  UMA VIDA  •  EVITE OS PERIGOS', {
      size: '11px', style: 'bold', color: '#7d3f23', align: 'center', depth: 33
    }).setOrigin(0.5);

    makeButton(this, 520, 548, 240, 54, 'VOLTAR AO MENU', () => {
...
function startGame() {
  try {
    const config = {
      ...
      scene: [BootScene, MenuScene, GameScene, ArrivalScene, FailureScene, VictoryScene, HowToPlayScene]
    };
