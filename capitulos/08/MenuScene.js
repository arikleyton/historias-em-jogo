class MenuScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('menuBg', 'backgroud-menu(cap-8).png');
    // carrega imagem da caravela que ficará no canto inferior direito
    this.load.image('caravela', 'caravela.png');
  }
  ...
  createMenuButtons() {
    // garante que o arquivo de estilo do jogo esteja carregado antes de criar os botões
    const ensureStylesheetLoaded = (filename = 'style.css') => {
      // evita carregar mais de uma vez
      if (document.querySelector(`link[data-game-style="${filename}"]`)) return;

      // tenta resolver o caminho relativo ao script atual (MenuScene.js)
      const scripts = Array.from(document.getElementsByTagName('script'));
      const script = scripts.find(s => s.src && s.src.endsWith('MenuScene.js')) || document.currentScript;
      let href = filename;
      try {
      ...
      link.rel = 'stylesheet';
      link.href = href;
      link.setAttribute('data-game-style', filename);
      document.head.appendChild(link);
    };

    ensureStylesheetLoaded('style.css');
    ...
    const html = `
      <div style="position:absolute; left:2%; top:48%; transform:translateY(-50%); display:flex; flex-direction:column; align-items:flex-start; gap:10px; z-index:8; pointer-events:auto;">
        ...
        <button id="home-btn" class="nav-btn home-btn" type="button" style="margin-top:2px;">Home</button>
      </div>`;
    ...
    storyBtn.addEventListener('click', () => {
      window.location.href = '../07/cap07.html';
    });
